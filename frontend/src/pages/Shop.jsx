...
import { CATEGORY_TREE, SORT_OPTIONS } from '../utils/helpers';
...
export default function Shop() {
  ...
  const categoryData = filterParams.category ? CATEGORY_TREE[filterParams.category] : null;
  const title = filterParams.productType
    ? filterParams.productType
    : filterParams.subCategory
      ? filterParams.subCategory
      : filterParams.category
        ? (categoryData?.label || filterParams.category)
        : 'All Products';

  const breadcrumb = [
    filterParams.category && { label: categoryData?.label || filterParams.category },
    filterParams.subCategory && { label: filterParams.subCategory },
    filterParams.productType && { label: filterParams.productType },
  ].filter(Boolean);
    ...
    <div className="pt-24 min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-5">
          ...
          {/* Quick Sub-Category chips — shown when category is selected */}
          {categoryData && (
            <div className="flex gap-2 mt-4 overflow-x-auto no-scrollbar pb-1">
              <button
                onClick={() => updateFilter({ subCategory: '', productType: '' })}
              ...
              {Object.entries(categoryData.subCategories).map(([subId, subData]) => (
                <button
                  key={subId}
                  onClick={() => updateFilter({ subCategory: filterParams.subCategory === subId ? '' : subId, productType: '' })}
                  className={`flex-shrink-0 text-xs px-4 py-2 rounded-full border font-semibold transition-all ${
                    filterParams.subCategory === subId
                      ? 'bg-red-600 text-white border-red-600'
                      : 'border-gray-200 text-gray-600 hover:border-red-400 hover:text-red-600 bg-white'
                  }`}
                >
          ...
          {/* Quick Type chips — shown when subCategory is selected */}
          {filterParams.subCategory && categoryData?.subCategories[filterParams.subCategory]?.types?.length > 0 && (
            <div className="flex gap-2 mt-2 overflow-x-auto no-scrollbar pb-1">
              <button
                onClick={() => updateFilter({ productType: '' })}
              ...
              {categoryData.subCategories[filterParams.subCategory].types.map(type => (
                <button
                  key={type}
                  onClick={() => updateFilter({ productType: filterParams.productType === type ? '' : type })}
                  className={`flex-shrink-0 text-xs px-3 py-1.5 rounded-full border font-medium transition-all whitespace-nowrap ${
                    filterParams.productType === type
                      ? 'bg-gray-800 text-white border-gray-800'
                      : 'border-gray-200 text-gray-500 hover:border-gray-400 bg-white'
                  }`}
                >
