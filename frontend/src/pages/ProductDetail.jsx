...
// ─── Fashion-Style Image Gallery ──────────────────────────────────────────────
const ANGLE_LABELS = ['Front', 'Back', 'Left', 'Right', 'Detail', 'Model', 'Close-up'];

function ImageGallery({ images, selectedColor, colorCode, onLightbox }) {
  const [activeIdx, setActiveIdx] = useState(0);
  const touchStartX = useRef(null);
  const thumbsRef = useRef(null);

  // Reset to first image whenever color changes
  ...
  const goLeft  = (e) => { e.stopPropagation(); setActiveIdx(i => Math.max(0, i - 1)); };
  const goRight = (e) => { e.stopPropagation(); setActiveIdx(i => Math.min(images.length - 1, i + 1)); };
  ...
  const onTouchEnd   = (e) => {
    ...
    if (diff > 40)  setActiveIdx(i => Math.min(images.length - 1, i + 1));
    if (diff < -40) setActiveIdx(i => Math.max(0, i - 1));
    touchStartX.current = null;
  };

  if (!images || images.length === 0) {
    return (
      <div className="w-full aspect-[3/4] bg-gray-100 rounded-3xl flex flex-col items-center justify-center text-gray-300 gap-3">
        <span className="text-7xl">📷</span>
        <p className="text-sm font-medium">No images added</p>
        {selectedColor && <p className="text-xs">Admin panel mein {selectedColor} color ke liye images add karo</p>}
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-3 w-full">
      ...
      <div
        ref={thumbsRef}
        className="hidden lg:flex flex-col gap-2.5 flex-shrink-0 overflow-y-auto no-scrollbar"
        style={{ width: 80, maxHeight: 540, scrollbarWidth: 'none' }}
      >
        {images.map((img, i) => (
          <button
            key={i}
            ...
            className={"relative flex-shrink-0 rounded-xl overflow-hidden border-2 transition-all duration-200 " + (
              i === activeIdx
                ...
                : 'border-gray-200 opacity-55 hover:opacity-90 hover:border-gray-400 hover:scale-105'
            )}
            style={{ width: 78, height: 90 }}
            title={ANGLE_LABELS[i] || `View ${i + 1}`}
          >
            <img
              src={img}
              alt={ANGLE_LABELS[i] || `View ${i + 1}`}
              className="w-full h-full object-cover"
              onError={e => { e.target.src = 'https://placehold.co/80x90?text=?'; }}
            />
            {/* View label badge */}
            <div className={"absolute bottom-0 left-0 right-0 text-center py-0.5 text-white text-[9px] font-semibold " + (
              i === activeIdx ? 'bg-red-500' : 'bg-black/50'
            )}>
              {ANGLE_LABELS[i] || `#${i + 1}`}
            </div>
      ...
      <div className="flex-1 flex flex-col gap-3 min-w-0">
        ...
        {selectedColor && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-gray-800">{selectedColor}</span>
            <span className="text-xs text-gray-400">— {images.length} photo{images.length > 1 ? 's' : ''}</span>
          </div>
        )}

        <div
          className="relative bg-white rounded-3xl overflow-hidden shadow-sm cursor-zoom-in group"
          style={{ aspectRatio: '3/4' }}
          onTouchStart={onTouchStart}
          onTouchEnd={onTouchEnd}
          onClick={() => onLightbox(activeIdx)}
        >
          <AnimatePresence mode="wait">
            <motion.img
              ...
              alt={`${selectedColor || 'Product'} - ${ANGLE_LABELS[activeIdx] || ''}`}
          ...
          {images.length > 1 && (
            <>
              <button onClick={goLeft}
                className={`absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/95 shadow-lg flex items-center justify-center transition-all ${
                  activeIdx === 0 ? 'opacity-30 cursor-not-allowed' : 'opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-600'
                }`}>
                <FiChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={goRight}
                className={`absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/95 shadow-lg flex items-center justify-center transition-all ${
                  activeIdx === images.length - 1 ? 'opacity-30 cursor-not-allowed' : 'opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-600'
                }`}>
          ...
          {/* Bottom bar: zoom + counter */}
          <div className="absolute bottom-0 left-0 right-0 px-3 py-2 bg-gradient-to-t from-black/60 to-transparent flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {/* Mobile thumbnails strip (horizontal, scrollable) */}
        {images.length > 1 && (
            ...
            {images.map((img, i) => (
              <button key={i} onClick={() => setActiveIdx(i)}
                className={"relative flex-shrink-0 rounded-xl overflow-hidden border-2 transition-all " + (
                  i === activeIdx ? 'border-red-500 scale-105' : 'border-gray-200 opacity-55'
                )}
                style={{ width: 64, height: 72 }}
              >
                <img src={img} alt="" className="w-full h-full object-cover" />
                <div className={"absolute bottom-0 left-0 right-0 py-0.5 text-[8px] font-semibold text-center " + (
                  i === activeIdx ? 'bg-red-500 text-white' : 'bg-black/50 text-white'
                )}>
                  {ANGLE_LABELS[i] || i + 1}
                </div>
              </button>
            ...
            {images.map((_, i) => (
              <button key={i} onClick={() => setActiveIdx(i)}
                className={"rounded-full transition-all duration-300 " + (
                  i === activeIdx ? 'w-6 h-2 bg-red-500' : 'w-2 h-2 bg-gray-300 hover:bg-gray-400'
                )} />
            ))}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          <div className="lg:col-span-6">
            <div className="relative">
              <ImageGallery
                key={selectedVariant?._id || selectedColor || 'default'}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-4">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2.5">
                    <span className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Color</span>
                    <div className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-full px-3 py-1">
                      {selectedVariant?.colorCode && (
                        <span className="w-3.5 h-3.5 rounded-full border border-white shadow-sm flex-shrink-0"
                          style={{ backgroundColor: selectedVariant.colorCode }} />
                      )}
                      <span className="text-sm font-bold text-red-600">{selectedColor}</span>
                    </div>
                    {selectedVariant?.images?.length > 0 && (
                      <span className="text-xs text-gray-400">{selectedVariant.images.length} photos</span>
                    )}
                  </div>
                  {selectedVariant?.sku && (
                    <span className="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-lg">SKU: {selectedVariant.sku}</span>
                  )}
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className={`relative overflow-hidden transition-all duration-200 ${
                          isSelected
                            ? 'ring-2 ring-red-500 ring-offset-2 scale-110 shadow-lg rounded-2xl'
                            : 'rounded-xl hover:scale-105 hover:shadow-md opacity-70 hover:opacity-100'
                        }`}
                          ...
                          {hasImg ? (
                            ...
                            <div className="w-full h-full border border-gray-200"
                              style={{ backgroundColor: variant.colorCode || COLOR_HEX[variant.colorName] || '#ddd', borderRadius: isSelected ? 12 : 8 }} />
                          )}
                          {isSelected && (
                            <div className="absolute inset-0 bg-red-600/10 flex items-end justify-center pb-1">
                        ...
                        <span className={`text-xs font-semibold transition-colors leading-tight text-center ${
                          isSelected ? 'text-red-600' : 'text-gray-500 group-hover:text-gray-800'
                        }`} style={{ maxWidth: 56 }}>
                          {variant.colorName}
                        </span>
                      </button>
                ...
                {selectedVariant?.images?.length === 0 && (
                  <p className="text-xs text-amber-600 mt-3 bg-amber-50 rounded-lg px-3 py-2">
                    ⚠️ Is color ki abhi koi photo nahi — admin panel mein 📷 button se add karo
                  </p>
                )}
              </div>
